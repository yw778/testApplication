...
static void print_usage()
{
    ...
    printf("    -r num_runs\n");
    printf("    -f dataset_multiply_factor");
    printf("    -g error_goal\n");
...
void parse_command_line_args(
    ...
    while ((c = getopt(argc, argv, "e:z:t:s:m:r:f:g:p:")) != -1) {
        switch (c) {
            case 'e':
                if (training_options)
...
// multiplies features in the dataset by a specified number of times.
// this function is mainly used to see the effects of adding more features
// per datapoint.
std::string multiplyDataset(int factor) {
    ifstream single_feats;
    ofstream multiple_feats;

    single_feats.open("data/shuffledfeats.dat");
    std::string new_feats_file_name = "data/" + patch::to_string(factor) + "xshuffledfeats.dat";
    multiple_feats.open(new_feats_file_name.c_str());

    string line;
    if (single_feats.is_open())
    {
        while ( getline(single_feats, line) )
        {
        ...
        single_feats.close();
        multiple_feats.close();
    }
    return new_feats_file_name;
}
