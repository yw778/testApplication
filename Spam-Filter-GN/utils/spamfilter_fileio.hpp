...
//read a text file and insert lines by factor times in vector
//return number of lines read; 0 if file failed to open
template <typename Type>
size_t readData(size_t size, const char* file_name, Type* vector, int factor) {
    ...
    size_t i = 0;
    for (Type val = 0; i < size && source_file >> val; i+=factor) {
